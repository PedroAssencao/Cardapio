...
namespace CárdapioV3_Tunado.DAL
{
    public class ProdutoEmpresaDAO
    {
        ...
        public void InsertProdutoEmpresa(ProdutoEmpresa novoProdutoEmpresa)
        {
            string query = "insert ProdutoEmpresa (EmpresaID, ProdutoID, CategoriaID) values (@EmpresaID, @ProdutoID, @CategoriaID)";

            int qtdInserida = _connection.Execute(query, novoProdutoEmpresa);
        }
        ...
        public void ApagarCategoria(ProdutoEmpresa deleteProduto)
        {
            string query = "delete from ProdutoEmpresa where CategoriaID=@CategoriaID";
